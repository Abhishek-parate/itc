...
═══════════════════════════════════ */
    ...
    /* ═══════════════════════════════════
   HERO BANNER  (info LEFT, image RIGHT)
═══════════════════════════════════ */
    .hero-band {
    ...
    /* RIGHT: Image card that bleeds up */
    .hero-right {
        position: relative;
        align-self: end
    }
    ...
    /* ═══════════════════════════════════
   DELIVERY INFO BAR  (below hero, 4 tiles)
═══════════════════════════════════ */
    .dbar {
        background: #fff;
    ...
    @media(max-width:700px) {
        ...
        .dbar-tile:nth-child(2) {
            border-right: none
        }

        .dbar-tile:nth-child(3),
        .dbar-tile:nth-child(4) {
            border-bottom: none
        }
    }
    ...
    /* ═══════════════════════════════════
   PAGE BODY  (2-col: content + sidebar)
═══════════════════════════════════ */
    .page-body {
        max-width: 1320px;
        margin: 0 auto;
    ...
    /* RELATED PRODUCTS (bottom of left col) */
    .related-grid {
        display: grid;
...
<body>
    ...
    <div class="page-body">
        ...
        <div class="left-col">
            ...
            <!-- OFFERS ROW -->
            <?php if(!empty($offers)): ?>
            <div class="offers-row au au3">
                <div class="offers-row-head">
                    <span class="offers-row-title">
                        <svg fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
                    ...
                    <button onclick="openOffersModal()" class="see-all">See All</button>
                </div>
                <div class="offers-scroll">
                    <?php foreach($offers as $offer): ?>
                    <div class="offer-chip">
                        <span class="offer-chip-icon"><?= htmlspecialchars($offer['icon']) ?></span>
                        <div>
                            <div class="offer-chip-text"><?= htmlspecialchars($offer['offer_text']) ?></div>
                            <?php if(!empty($offer['valid_note'])): ?>
                            <div class="offer-chip-note"><?= htmlspecialchars($offer['valid_note']) ?></div>
                            <?php endif; ?>
                        </div>
                    </div>
                    <?php endforeach; ?>
        ...
        <div class="right-col">
            ...
            <!-- PRICE + BUY CARD -->
            <div class="pcard au au1">
                <div class="pcard-top">
                    <div class="pcard-mrp">MRP <?= formatINR($mrpDisplay) ?> (Incl. taxes)</div>
                    <div id="side-price" class="pcard-price"><?= formatINR($product['price_min']) ?></div>
                    <div class="pcard-gst">+ <?= $product['gst_percent'] ?>% GST
                        <?php if($savings > 0): ?><span class="pcard-save">Save
                            <?= formatINR($savings) ?></span><?php endif; ?>
                    </div>
                </div>
                <div class="pcard-body">
                    <div class="pcard-qty-lbl">Select Quantity</div>
                    <div class="pcard-qty">
                        <div class="qty-box">
                            <button onclick="decreaseQuantity()"
                                style="width:32px;height:36px;color:var(--navy);font-size:16px">−</button>
                            <input type="text" id="sideQtyDisplay" value="1" readonly
                                style="width:38px;height:36px;text-align:center;font-size:13px;font-weight:800;color:var(--navy);border:none;outline:none;background:#fff;border-left:2px solid var(--navy);border-right:2px solid var(--navy);font-family:'Sora',sans-serif">
                            <button onclick="increaseQuantity()"
                                style="width:32px;height:36px;color:var(--navy);font-size:16px">+</button>
                        </div>
                        <span class="pcard-min">Min: <?= $product['min_order'] ?></span>
                    </div>
                    <button onclick="addToCart()" class="pcard-btn-cart">
                    ...
                    <button onclick="openQuoteModal()" class="pcard-btn-quote">REQUEST QUOTE</button>
                    <div class="pcard-links">
                        <button class="pcard-link"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                            </svg>Call +91 84688-51160</button>
                        <button onclick="buyOnChat()" class="pcard-link wa"><svg fill="currentColor"
                                viewBox="0 0 24 24">
                                <path
                                    d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                            </svg>Buy on WhatsApp</button>
                        <button class="pcard-link"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                            </svg>Ask for Bulk Quote</button>
                    </div>
                </div>
            </div>

            <!-- META TILES -->
            <div class="meta-tiles au au2">
                <div class="meta-tile">
                    <div class="mt-lbl">Min. Order</div>
                    <div class="mt-val"><?= $product['min_order'] ?> <?= htmlspecialchars($product['min_order_unit']) ?>
                    </div>
                </div>
                <div class="meta-tile">
                    <div class="mt-lbl">Delivery</div>
                    <div class="mt-val"><?= htmlspecialchars($product['delivery_days']) ?></div>
                </div>
                <div class="meta-tile">
                    <div class="mt-lbl">Warranty</div>
                    <div class="mt-val"><?= htmlspecialchars($product['warranty']) ?></div>
                </div>
                <div class="meta-tile">
                    <div class="mt-lbl">Certification</div>
                    <div class="mt-val"><?= htmlspecialchars($product['certification']) ?></div>
                </div>
            </div>
